...
abstract class RestaurantController extends ControllerBase {
    ...
    public static function searchRestaurants($conditionCallback = NULL, $config = array()) {
      /*  $query = db_select('food_restaurant', 'fr')
			...
			$query = db_select('food_restaurant', 'fr')
            ->fields('fr', array('restaurant_id', 'created_time',   'status', 'name', 'address_line1', 'address_line2', 'city', 'state', 'postal_code', 'country', 'latitude', 'longitude',   'about_detailed',  'speciality', 'order_types', 'timings', 'deals', 'tax_pct','image_fid'));
