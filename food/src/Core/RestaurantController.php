...
abstract class RestaurantController extends ControllerBase {
    ...
    public static function searchRestaurants($conditionCallback = NULL, $config = array()) {
      /*  $query = db_select('food_restaurant', 'fr') 
			...
			$query = db_select('food_restaurant', 'fr')
            ->fields('fr', array('restaurant_id', 'owner_user_id', 'created_time', 'created', 'changed', 'status', 'image_fid', 'name', 'phone_number', 'fax_number', 'email', 'address_line1', 'address_line2', 'city', 'state', 'postal_code', 'country', 'latitude', 'longitude',  'speciality', 'order_contact_details', 'order_types', 'order_constraints', 'timings', 'delivery_area_type', 'delivery_radius', 'delivery_polygon', 'user_payment_settings',   'platform_settings', 'deals', 'tax_pct', 'url_slug', 'featured_restaurant', 'payment_accept_mode'));
